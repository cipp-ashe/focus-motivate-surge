...
export const useHabitEvents = (
  ...
  const processedTemplateIdsRef = useRef(new Set<string>());
  ...
  useEffect(() => {
    const clearProcessedTemplatesAtMidnight = () => {
      ...
      setTimeout(() => {
        ...
        processedTemplateIdsRef.current.clear();
        setupResetTimer();
      }, timeUntilMidnight);
    };
  ...
  useEffect(() => {
    ...
    const unsubscribers = [
      ...
      eventBus.on('habit:template-add', (templateId: string) => {
        ...
        // Skip if already processed this template today
        if (processedTemplateIdsRef.current.has(templateId)) {
          console.log(`Template ${templateId} was already processed today, skipping duplicate event`);
          return;
        }
        ...
        // Mark as processed
        processedTemplateIdsRef.current.add(templateId);
        ...
        try {
          ...
          }, 1000); // Longer timeout to prevent multiple additions
        }
      }),
