...
import { MetricType, DayOfWeek, ActiveTemplate } from '@/types/habit';
...
export const useUnifiedHabitEvents = () => {
  ...
  const updateHabitTemplate = useCallback(
    (templateData: Partial<ActiveTemplate> & { templateId: string }) => {
      eventManager.emit('habit:template-update', templateData);
    },
    []
  );

  const deleteHabitTemplate = useCallback(
    (templateId: string, isOriginatingAction: boolean = true) => {
  ...
  const addHabitTemplate = useCallback(
    (template: Partial<ActiveTemplate> & { templateId: string }) => {
  ...
  // Journal integration
  const openHabitJournal = useCallback(
    (
      habitId: string,
      habitName: string,
      description?: string,
  ...
  const createHabitNote = useCallback(
    (habitId: string, habitName: string, content: string, templateId?: string) => {
  ...
  const createHabitJournal = useCallback(
    (habitId: string, habitName: string, content: string, templateId?: string, date?: string) => {
