...
import { Note, NoteTag, NoteType } from '@/types/notes';
...
export const NoteEditor: React.FC<NoteEditorProps> = ({ note, onAddTag, onRemoveTag }) => {
  const { addNote, updateNote, clearSelectedNote, toggleFavorite } = useNotes();
  ...
  const handleAddTag = useCallback(
    (tagName: string, color: string = 'default') => {
      if (!note) return;
    ...
    <div className="flex flex-col h-full animate-fade-in" onKeyDown={handleKeyDown}>
      <div className="flex items-center justify-between">
        <div className="flex items-center">
          <Button variant="ghost" size="icon" onClick={handleBack} className="mr-2 md:hidden">
        ...
        <div className="flex items-center space-x-2">
          {note && (
            <Button
              ...
              className={cn('h-8 w-8', note.favorite && 'text-yellow-500')}
            >
              <Star className={cn('h-4 w-4', note.favorite && 'fill-yellow-400')} />
            </Button>
          )}
      ...
      <div className="flex items-center mt-3 mb-4">
        {note ? (
          <>
            <NoteTagList tags={note.tags} onRemove={handleRemoveTag} className="flex-1" />
            <NoteTagInput onAddTag={handleAddTag} />
          </>
        ) : (
          <p className="text-sm text-muted-foreground">Tags can be added after creating the note</p>
        )}
      </div>
