...
export const TimerCircle = memo(
  ({
    ...
    const sizeClasses =
      size === 'large' ? 'w-60 h-60 md:w-64 md:h-64' : 'w-48 h-48 md:w-52 md:h-52';

    return (
      <div
        className={cn('relative mx-auto', sizeClasses, onClick && 'cursor-pointer')}
        onClick={onClick}
        {...a11yProps}
      >
        <svg className="timer-circle transform -rotate-90" viewBox="0 0 100 100" aria-hidden="true">
          {/* Background circle */}
          <circle
