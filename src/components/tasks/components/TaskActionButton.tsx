...
export const TaskActionButton: React.FC<TaskActionButtonProps> = ({
  ...
  function StatusDropdownMenu({ task, onTaskAction }: { task: Task; onTaskAction: (e: React.MouseEvent<HTMLButtonElement>) => void }) {
    ...
    const getStatusInfo = (status: TaskStatus = 'pending') => {
      switch (status) {
        ...
        case 'in-progress':
          return { label: 'In Progress', icon: <Loader2 className="h-3.5 w-3.5 mr-1 animate-spin" />, className: "bg-amber-500/10 hover:bg-amber-500/20 text-amber-500" };
        case 'completed':
        ...
        <DropdownMenuContent align="end" className="w-36 bg-background">
          <DropdownMenuItem
            onClick={(e) => {
              ...
              button.setAttribute('data-action-type', 'status-pending');
              onTaskAction({ ...e, currentTarget: button } as any);
            }}
          ...
          <DropdownMenuItem
            onClick={(e) => {
              const button = document.createElement('button');
              button.setAttribute('data-action-type', 'status-in-progress');
              onTaskAction({ ...e, currentTarget: button } as any);
            }}
            className="text-xs cursor-pointer"
          >
          ...
          <DropdownMenuItem
            onClick={(e) => {
              const button = document.createElement('button');
              button.setAttribute('data-action-type', 'status-completed');
              onTaskAction({ ...e, currentTarget: button } as any);
            }}
          ...
          <DropdownMenuItem
            onClick={(e) => {
              const button = document.createElement('button');
              button.setAttribute('data-action-type', 'status-dismissed');
              onTaskAction({ ...e, currentTarget: button } as any);
            }}
