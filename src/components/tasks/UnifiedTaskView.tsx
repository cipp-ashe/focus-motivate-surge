...
export const UnifiedTaskView: React.FC<UnifiedTaskViewProps> = ({
  ...
  const handleTaskComplete = useCallback(
    (data: { taskId: string; metrics?: any }) => {
  ...
  const getFilteredTasks = useCallback(() => {
    ...
    // Check for duplicate tasks (same ID in both active and completed arrays)
    const duplicateIds = new Set();
    const allTaskIds = [...activeTasks, ...completedTasks].map((t) => t.id);

    allTaskIds.forEach((id) => {
      if (activeTasks.some((t) => t.id === id) && completedTasks.some((t) => t.id === id)) {
        duplicateIds.add(id);
      }
    });
    ...
    if (duplicateIds.size > 0) {
      console.warn(
        'UnifiedTaskView: Found duplicate task IDs in both active and completed arrays:',
        Array.from(duplicateIds)
      );
    }

    // Ensure we don't have duplicate tasks in the current list
    const uniqueTasks = currentTasks.reduce((acc, task) => {
      // If we already have this task ID, skip it
      if (acc.some((t) => t.id === task.id)) {
        console.warn(
          `UnifiedTaskView: Skipping duplicate task with ID ${task.id} in ${activeTab} tasks`
        );
        return acc;
      }
      return [...acc, task];
    ...
    if (uniqueTasks.length !== currentTasks.length) {
      console.warn(
        `UnifiedTaskView: Removed ${
          currentTasks.length - uniqueTasks.length
        } duplicate tasks from ${activeTab} tasks`
      );
    }

    // Apply task type filter to the deduplicated list
    if (activeTaskType === 'all') return uniqueTasks;
    return uniqueTasks.filter((task) => task.taskType === activeTaskType);
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      >
        ...
        <TabsContent
          value="active"
          className="flex-1 overflow-hidden p-0 m-0 data-[state=active]:border-0"
        >
