  ...
  switch (action.type) {
    ...
    case 'COMPLETE_TASK': {
      ...
      const formattedMetrics: TaskMetrics = action.payload.metrics ? {
        ...
        timeSpent: action.payload.metrics.actualDuration || action.payload.metrics.timeSpent || 0,
        timeElapsed: action.payload.metrics.actualDuration || 0,
        pauseCount: action.payload.metrics.pauseCount || 0,
        completionDate: new Date().toISOString(),
        // Timer-specific metrics - preserve all of them for timer tasks
        expectedTime: action.payload.metrics.expectedTime || 0,
        actualDuration: action.payload.metrics.actualDuration || 0,
        favoriteQuotes: Array.isArray(action.payload.metrics.favoriteQuotes) 
          ? action.payload.metrics.favoriteQuotes 
          : [],
        pausedTime: action.payload.metrics.pausedTime || 0,
        extensionTime: action.payload.metrics.extensionTime || 0,
        netEffectiveTime: action.payload.metrics.netEffectiveTime || 0,
        efficiencyRatio: action.payload.metrics.efficiencyRatio || 0,
        completionStatus: action.payload.metrics.completionStatus || 'Completed',
      } : {
        timeSpent: 0,
        timeElapsed: 0,
        pauseCount: 0,
